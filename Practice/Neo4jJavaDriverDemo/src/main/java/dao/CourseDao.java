...
public class CourseDao {
	...
	public CourseDao(Driver driver) {
		this.driver = driver;
		this.sessionConfig = SessionConfig.builder().withDatabase("course").build();
	}
	 ...
	 * 
	 * @param courseId
	 * @param deptId   String query = "MATCH (c: Course {courseID: $courseId}) MATCH
	 *                 (d: Department {deptID: $deptId}) MERGE
	 *                 (c)-[:BELONGS_TO]->(d)";
	 */
	 ...
	 * @return List of courses if found, empty list otherwise String query = "MATCH
	 *         (c: Course)-[:BELONGS_TO]->(d: Department) WHERE d.deptID = $id
	 *         RETURN c";
	 */
